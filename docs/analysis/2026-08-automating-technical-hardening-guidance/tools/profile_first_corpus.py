#!/usr/bin/env python3
"""
profile_first_corpus.py: write the profile-first corpus into examples/profile-first/oscal/.
...
OUT_DIR = os.path.join(SITE_ROOT, "examples", "profile-first", "oscal")
...
def uid(seed: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, "tfg-profile-first:" + seed))
...
def build_cis(bench) -> tuple[dict, dict, list]:
    ...
    catalog = {"catalog": {
        ...
        "metadata": {
            ...
            "remarks": ("Generated by tools/profile_first_corpus.py from the "
...
def build_mapping(bench, rules, catalog_file: str) -> dict:
    ...
    return {"mapping-collection": {
        ...
        "metadata": {
            ...
            "version": bench["version"],
            "oscal-version": OSCAL_VERSION,
            "remarks": ("Generated by tools/profile_first_corpus.py from the SP 800-53 "
...
def build_stig(platform: str) -> dict:
    ...
    return {"profile": {
        ...
        "metadata": {
            ...
            "remarks": (
                f"Generated by tools/profile_first_corpus.py from the STIG named in "
...
def main() -> int:
    ...
    if "--check" in sys.argv:
        ...
        if stale or extra:
            print(f"examples/profile-first/oscal is stale: {stale + extra}. "
                  f"Run tools/profile_first_corpus.py")
            return 1
        print(f"examples/profile-first/oscal is current, {len(files)} files")
        return 0
